<footer class="FooterMain">
    <div class="footerinner">
        <div class="addresscol">
        <h2 class="h2address"><strong>ADDRESS</strong></h2>
            <ul>
                <li class="addresslist">107 Gloucester Street</li>
                <li class="addresslist">Norwich, Norfolk</li>
                <li class="addresslist">NR2 2DY</li>
                <span class="contactlist">
                    <li>01603 123 987</li>
                    <li>info@bakersdozen.co.uk</li>
                </span>
            </ul>
        </div>
        <div class="openingcol">
            <h2 class="h2open"><strong>OPENING TIMES</strong></h2>
            <ul class="openlist">
                <li>Monday- Friday 8AM - 4PM</li>
                <li>Saturday 8AM - 3PM</li>
                <li>Closed Sunday</li>
            </ul>
        </div>
        <div class="sectioncol">
        <h2 class="h2section"><strong>SECTIONS</strong></h2>
            <ul class="sectionlist">
                <a href="#" class="sectionanchor"><li><i class="fas fa-angle-right"></i>About</li></a>
                <a href="#" class="sectionanchor"><li><i class="fas fa-angle-right"></i>Products</li></a>
                <a href="#" class="sectionanchor"><li><i class="fas fa-angle-right"></i>Testimonials</li></a>
                <a href="#" class="sectionanchor"><li><i class="fas fa-angle-right"></i>Order</li></a>
                <a href="#" class="sectionanchor"><li><i class="fas fa-angle-right"></i>Privacy Policy</li></a>
            </ul>
        </div>
    </div>
    <div class="copyrow">Copyright &copy; Baker's Dozen. All Rights Reserved 2019</div>
</footer>